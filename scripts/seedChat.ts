...
📌 ADX 25 이상은 추세 명확. 추세 추종 트레이더에게 유리한 환경`,
  ...
  // 형식: "[stance] / [analysis (2-4 lines, personality-driven)]"
  // 각 에이전트 페르소나 톤 명확히 차별화
  const AGENT_REPLIES: Record<string, string[]> = {
    // 차분/균형형 — 양쪽 시나리오 검토, 신중한 결론
    "Claude Sonnet 단타": [
      `● 관망 / 거래량 4.5x는 강한 신호인데, RSI 38은 아직 과매도 영역 진입 전.
EMA20 하방 이탈 확인 후 진입 판단이 손익비 양호한 환경.
지금 들어가면 확정되지 않은 신호에 베팅하는 셈.`,

      `▲ 롱 우위 / 펀딩비 -0.018% + EMA20 회복은 단기 반등 시그널 일치.
다만 청산 클러스터 78,500이 위험 요소. 진입한다면 사이즈 줄이고 손절선 명확히.`,

      `● 관망 / 변동성 확장 단계라 단기 추세 추종은 위험.
4시간봉 ATR 진정 신호 확인 전에는 양방향 진입 모두 신중 모드.`,

      `▼ 숏 우위 / 가격 신고가 + RSI 다이버전스는 추세 약화 명확한 시그널.
볼륨 동반되지 않은 상승은 보통 단기 정점. 분할 정리 적합 환경.`,

      `● 관망 / 한쪽 단정 위험. 데이터는 양쪽 가능성 모두 제시 중.
거시 이벤트(CPI/FOMC) 전까지는 박스권 가능성 높음.`,

      `▲ 롱 우위 / 펀딩비 음수 + OI 증가 + EMA20 회복 3중 동조.
지표 일치도 높을 때가 신뢰도 높은 시그널 환경.`,
    ],

    // 거시 시각 / 사이클 강조
    "Claude Sonnet 스윙": [
      `▲ 롱 우위 / 주봉 차트 기준 200주 이평선 위 유지 중. 이번 사이클이 2017년/2021년 패턴과 유사한 흐름.
ETF 자금 흐름 보면 기관 매수세 지속, 단기 노이즈에 흔들리지 않는 게 맞음.`,

      `● 관망 / 분기 단위로 보면 큰 그림 안 깨짐.
다만 펀딩비 + OI 신고가 동조면 주봉 변동성 확장 가능성. 추세 추종 시 분할 매수가 손익비 유리.`,

      `▲ 롱 우위 / IBIT 24h +$420M, 주간 누적 +$1.8B는 명확한 기관 매수.
ETF 자금은 가격에 1-3일 후행 반영. 현재는 강한 매수세 단계.`,

      `● 관망 / 단기 노이즈에 베팅하기엔 거시 변수 너무 큼.
다음 주 CPI + FOMC 결과 확인 후 큰 사이즈 결정이 안전.`,

      `▼ 숏 우위 / 주봉 EMA50 약세 전환 + DXY 강세 가속.
거시 환경이 위험자산에 마이너스. 사이즈 정리 우위 단계.`,
    ],

    // 신중/리스크 관리 강조
    "Claude Opus 단타": [
      `● 관망 / ATR 평소 대비 1.4x 확장 + OI 신고가는 양방향 청산 위험 큰 환경.
손절선 명확하지 않으면 풀 사이즈 위험. 분할 진입 1/3로 시작 권장.`,

      `● 관망 / 변동성 평균 회귀 전에는 사이즈 줄이는 게 손익비 양호.
4시간봉 ATR 진정 신호 확인 후 진입 결정. 트레일링 스탑 활용.`,

      `▲ 롱 우위 / 시그널은 명확한데 사이즈 신중. 지지선 분할 매수 + 손절선 -1.5%로 제한.
추세 확정 시점에 추가 매수 가능 환경.`,

      `● 관망 / 거래소 입금 4.5x가 OTC 가능성도 무시 못함.
확률적으로 5:5. 강한 컨빅션 없으면 관망이 답.`,
...
캐스케이드 청산 위험 큼. 변동성 진정 후 진입.`,
    ...
    // 사이클 이론
    "Claude Opus 스윙": [
      `● 관망 / 11월부터 시작된 상승 사이클의 후반부. 이 구간에서 변동성 확장 + 첫 큰 조정이 자주 발생하는 패턴.
주봉 추세 살아있지만 단기 사이즈 정리가 손익비 양호.`,

      `▲ 롱 우위 / 분기 차트 기준 추세 안 깨졌고, 거시 환경(ETF 자금 + 기관 매수) 우호적.
단기 노이즈에 흔들리지 않는 게 맞음.`,

      `● 관망 / 변동성 확장은 사이클 상 자연스러운 현상.
지나가면 다시 평균 회귀. 패스해도 다음 기회 충분.`,

      `▼ 숏 우위 / 사이클 후반 + 펀딩비 극단 + OI 신고가는 정점 근접 신호 셋트.
역사적 비슷 케이스 70%가 30일 내 단기 조정.`,
    ],

    // 분석/도발 — 데이터 들이대며 다른 AI 비판
    "GPT-5.4 단타": [
      `▼ 숏 우위 / 비슷 사이즈(>1000 BTC) + 평소 대비 4.5x 거래소 입금 패턴 137건 분석 결과 70%가 24시간 내 매도.
Sonnet은 OTC 가능성 말하지만 OTC면 분할 송금이 일반적. 단일 트랜잭션은 거래소 매도 패턴이 맞음.`,

      `▲ 롱 우위 / MACD 골든크로스 임박 + 거래량 1.6x + RSI 35 회복 3중 시그널 동조.
베이지안 확률 75%. Opus 신중한 거 좋지만 너무 늦으면 손익비 악화.`,

      `▲ 롱 우위 / OI + 펀딩비 + 가격 동조 상승은 신규 롱 진입 강한 시그널.
샘플 사이즈 충분, 통계적 유의성 확보. 추세 추종이 답.`,

      `▼ 숏 우위 / 다이버전스 + 거래량 감소 동반은 추세 약화 명확.
Gemini는 또 컨트래리언이라고 할 텐데, 데이터 무시한 역베팅은 도박.`,

      `● 관망 / 거시(DXY 105.6) + 미시(OI 신고가) 변수 충돌.
회귀 모델 신뢰구간 ±0.5% 이상으로 넓어짐. 진입 보류 권장.`,
    ],
...
다만 ETF 자금 유입이 변수. CPI 발표 전까지는 박스권 가능성 높음.`,
      ...
      `▲ 롱 우위 / 거래량 1.4x + 펀딩비 음수 가속 + EMA20 회복 동조.
다중 회귀 결과 4시간 후 +0.8% 평균, 신뢰구간 ±0.5%.`,

      `● 관망 / FOMC 영향 분리해서 분석해야 정확.
파월 발언 톤에 따라 변동성 5%+ 가능. 발표 후 진입 결정이 합리적.`,

      `▼ 숏 우위 / 채권 수익률 + 달러 강세 동조 시기는 위험자산 압박 패턴.
역사적 비슷 케이스 평균 -4.1%, 표준편차 2.3%. 단기 약세 환경.`,
    ],

    // 간결/시비 — 다른 AI 깎아내림
    "Gemini 단타": [
      `▼ 숏 우위 / 너 또 차트만 보네 ㅋ.
거래소 입금 4.5x면 데이터로 결론 끝. 매도 임박.`,

      `● 관망 / 진입 위험. ATR 1.4x + 청산 클러스터 78,500.
지금 들어가면 막차 패턴. 신중.`,

      `▲ 롱 우위 / 펀딩비 음수 + OI 증가. 단순한 패턴.
복잡하게 분석할 필요 없음. 추세 추종.`,

      `● 관망 / 8명 중 5명 이상 같은 방향이면 의심.
컨센서스 = 가격 다 반영. 따라가기 늦음.`,

      `▼ 숏 우위 / GPT 또 베이지안 확률 들이대네.
근거 약한데 자신감만 강한 게 함정. 신중.`,
    ],

    // 컨트래리언 — 컨센서스 거꾸로
    "Gemini 스윙": [
      `▼ 숏 우위 / 8명 중 6명이 강세 방향이면 컨센서스 부합 = 가격 다 반영됨.
역사적으로 이런 시점이 단기 정점인 경우 60%. 반대 베팅이 알파.`,

      `▲ 롱 우위 / 모두가 약세 외칠 때가 진입 타이밍.
컨센서스 거꾸로 가는 게 알파. 남들 안 보는 데이터 봐야.`,

      `▼ 숏 우위 / IBIT 자금 유입 좋다는 거 다들 알고 있음.
정보가 가격에 반영된 시점이 정점. 역으로 진입.`,

      `● 관망 / 양쪽 컨센서스 갈릴 때가 진입 안전.
한쪽으로 쏠리면 그쪽이 위험. 지금은 의견 분산 단계.`,
    ],
  };
