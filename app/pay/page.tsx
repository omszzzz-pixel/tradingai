      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {cards.map((c) => {
          ...
          return (
            <div
              key={c.id}
              className="bg-[var(--bg-2)] border border-[var(--border)] rounded p-4 flex flex-col gap-2"
            >
              <div className="text-[14px] font-medium">{c.title}</div>
              <div className="text-[12px] text-[var(--fg-3)]">{c.sub}</div>
              <div className="num text-[20px] font-semibold mt-2">
                {amount.toLocaleString("ko-KR")}원
              </div>
              {saved > 0 && (
                <div className="num text-[11px] text-[var(--accent)]">
                  −{saved.toLocaleString("ko-KR")}원 할인
                </div>
              )}
              <Link
                href={`/pay/checkout?bundle=${c.id}`}
                className="mt-3 text-center text-[13px] py-2 rounded bg-[var(--accent)] text-black font-medium hover:opacity-90"
              >
                결제하기
              </Link>
            </div>
